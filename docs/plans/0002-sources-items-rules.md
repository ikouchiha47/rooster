...
**Dependency found while starting T6.3: the preview has no data source.** `RuleWriter.dryRun(rule, seeds)` evaluates over *caller-supplied* seeds and persists nothing — correct as far as it goes, but nothing loads those seeds from the store, so the mockups' "6 events would match · last 7 days" has nothing to run on. The bounded history query was assumed to be slice 5's; it is actually a hard prerequisite for the preview and is being built first as `feat/rule-preview`: a date-bounded, scan-capped loader that narrows in SQL where the condition makes it cheap and indexed (source, tag, mention) and leaves regex and comparisons to the evaluator, exactly as ADR §12 divides it. It must also degrade on a series condition rather than throwing into the UI, since `RuleEvaluator` deliberately throws for those until slice 5.
