...
- [x] T1.4 `SourceRepository` (was `RuleRepository`, behaviour unchanged incl. locked seeds) and a new `RuleRepository` that is **storage-only** — condition/action validation moves to slice 2, because that is where the condition schema exists (P7 depends on the predicate language). (S1)
...
- 2026-09-16 — Slice 1 review + cleanup pass in flight (oracle review, then bounded fixes). Known open items: local variables still named `rule`/`rules` in a few functions; the `TagSourceKind` alias in `FeedIngestor` resolving a `SourceKind` name collision; `Migrations.CURRENT_VERSION` is `12` while `AppDatabase.version` is `11` and is unused.
